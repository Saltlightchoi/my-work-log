...
# [핵심 수정] Secrets에 저장된 URL을 직접 변수로 가져옵니다.
# 만약 로컬 테스트 중이라면 직접 주소를 입력해도 됩니다.
try:
    SHEET_URL = st.secrets["connections"]["gsheets"]["spreadsheet"]
except:
    SHEET_URL = "" # Secrets 설정이 없을 경우 대비

def get_users():
    # 명시적으로 spreadsheet 주소를 전달하여 에러를 방지합니다.
    return conn.read(spreadsheet=SHEET_URL, worksheet="users", ttl=0)

def get_data():
    return conn.read(spreadsheet=SHEET_URL, worksheet="data", ttl=0)
    ...
    try:
        ...
        if mode == "➕ 작성":
            ...
            if not work_df.empty:
                ...
                st.sidebar.markdown(f"<div class='delete-box'><strong>삭제 항목 정보:</strong><br>📅 날짜: {selected_row['날짜']}<br>📝 내용: {selected_row['업무내용']}</div>", unsafe_allow_html=True)
        ...
        display_df = work_df[work_df.apply(lambda r: search_keyword.lower() in str(r).lower(), axis=1)] if search_keyword else work_df
