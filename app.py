...
st.markdown("""
    <style>
        ...
        [data-testid="stSidebar"] [data-testid="stVerticalBlock"] { gap: 0.2rem !important; padding-top: 0.5rem !important; }
        
        /* 사이드바 로그인 정보 및 로그아웃 버튼 최소화 */
        .sidebar-user-text { font-size: 11px !important; color: #aaaaaa; margin-top: 5px !important; }
...
if not st.session_state['logged_in']:
    ...
    try:
        ...
        if mode == "➕ 작성":
            ...
            if not df.empty:
                ...
                with st.sidebar.form("edit_form"):
                    ...
                    e_content = st.text_area("내용 수정", value=df.loc[edit_idx, "업무내용"], height=120)
            ...
            if not df.empty:
                ...
                st.sidebar.warning(f"⚠️ 선택: {df.loc[del_idx, '업무내용'][:15]}...")
        ...
        with head_c2:
            ...
            st.download_button(label="📥 엑셀 다운", data=csv_data, file_name=f"log_{datetime.now().strftime('%m%d')}.csv")
        ...
        st.markdown("""
            <div class='info-box'>
                📎 <b>사진 확인:</b> 경로 클릭 → <b>Ctrl+C</b> → <b>[윈도우+R]</b> 창에 붙여넣으세요.
            </div>
        """, unsafe_allow_html=True)

        # 데이터프레임 출력 (문법 오류 수정 완료)
        st.dataframe(
            display_df,
